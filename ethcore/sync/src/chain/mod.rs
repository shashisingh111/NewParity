...
pub type Trx = HashMap<String, HashMap<H256,Duration>>;
...
impl ChainSync {
	...
	pub fn new(
	 ...
	 pub fn add(&mut self ,enode: String, trx: H256,time: Duration)
    {
        if !&self.tx.contains_key(&enode)
        {   let mut hash = HashMap::new();
            hash.insert(trx,time);
            &self.tx.insert(enode,hash);
        }
        else
        {
            if let  Some(x) =  self.tx.get(&enode)
             { 
                let mut hash1=x.clone();
                  hash1.insert(trx,time);
                  *(self.tx.get_mut(&enode).unwrap())=hash1;
            }
        }
    }

	// @shashi... print txrecord

	pub fn print_tx(&self)
    {
        for (key, value) in &self.tx
        {
            println!("{},{:#?}",key,value);
        }
    }
	...
	fn send_status(&mut self, io: &mut SyncIo, peer: PeerId) -> Result<(), network::Error> {
		println!("Send status to peer...{}",peer);
	...
	pub fn chain_new_blocks(&mut self, io: &mut SyncIo, _imported: &[H256], invalid: &[H256], enacted: &[H256], _retracted: &[H256], sealed: &[H256], proposed: &[Bytes]) {
		println!("Sent new blocks to peers....");
